...
static int scan_fs(queue_t *in_q, queue_t *out_q) {
        conf_t *conf = get_conf();

        in_queue  = in_q;
        out_queue = out_q;

        /* set maximum number of open files for nftw to a half of
           descriptor table size for current process */
        struct rlimit rlim;
        if (getrlimit(RLIMIT_NOFILE, &rlim) == -1) {
                return -1;
        }

        /* rlim_cur value will be +1 higher than actual
...
void file_system_scanner(queue_t *demotion_queue, queue_t *promotion_queue) {
        ...
        for (;;) {
                if (scan_fs(promotion_queue, demotion_queue) == -1) {
